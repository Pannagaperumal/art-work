...
exports.processPayment = asyncErrorHandler(async (req, res, next) => {
  ...
  const params = initializePaymentParams(amount, email, phoneNo, req);
...
function initializePaymentParams(amount, email, phoneNo, req) {
  return {
...
async function generatePaytmChecksum(params) {
  return await paytm.generateSignature(params, process.env.PAYTM_MERCHANT_KEY);
}
